	...
	class ezSQL_pdo extends ezSQLcore
	{
		...
		*/
		...
		function connect()
		{
			...
			if ( !isset($this->dsn,$this->user,$this->password))
			{
				$this->register_error('Require $dsn and $user and $password to create a connection in '.__FILE__.' on line '.__LINE__);
				$this->show_errors ? trigger_error('Require $dsn and $user and $password to create a connection', 	E_USER_ERROR) : null;
			}
			...
			catch (PDOException $e) 
			{
    			...
    			$this->show_errors ? trigger_error($e->getMessage(),E_USER_ERROR) : null;
			}
			...
			else
			{
				...
				for ( $i=0 ; $i < $col_count ; $i++ )
				{
					...
					{					
						...
						$this->col_info[$i]->type =  isset($meta['native_type'])?$meta['native_type']:'';
