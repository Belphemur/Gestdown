...
# * htpasstool v1.0.3
# *
# * Copyright (C) 2006,2007  Ward Vandewege (ward AT pong DOT be)
# *
...
$admin_email = 'antoineaf@gmail.com';
...
{
    $repeat=array('iVBORw0KGgoAAAANSUhEUgAAAA4AAAALCAYAAABPhbxiAAAABmJLR0QA','AAAACXBIWXMAAAsNAAALDQHtB8AsAAAAB3RJTUUH1AQFFh','AAAFRJREFUeJxjZMAE');
    $imagesEncoded = Array(
            "padlock"     => "{$repeat[0]}AAAAAAD5Q7t/{$repeat[1]}Q155Nu3w{$repeat[2]}/5HYjFjkGRgYGBiYcGhiROMTBOgKcWpEt5GBgYGBjVhbYIAVagM65sGmGNnz+PyDEUjYnEoUwKrx/3/CgUm2jVTxIwcOTczYBAEMQw8nZ2fMggAAAABJRU5ErkJggg==",
            "padlock-open"=> "{$repeat[0]}/wAAAAAzJ3zz{$repeat[1]}YN/ae0ww{$repeat[2]}TGj0fySMFzAisZE1EKUZWSNWPhOaBAsDAwMrsabCACsDqn9gmAebYnT/4AKM6ALoTiUaYNX4/z/hwCPbRqr4kQOHJmZsggBvtxMm/Y4B+AAAAABJRU5ErkJggg==",
    );
    ...
    <body>
        ...
        <div class="centered">
            <table id="outside" width="600">
                ...
                $newlogin = isset($_REQUEST['newlogin'])?$_REQUEST['newlogin']:'';
                $password = isset($_REQUEST['password'])?$_REQUEST['password']:'';
                $password2 = isset($_REQUEST['password2'])?$_REQUEST['password2']:'';
                $assign_random_password = isset($_REQUEST['$assign_random_password'])?$_REQUEST['$assign_random_password']:'';
                        ...
                        <table width="100%">
                            ...
                            <tr>
                                <td>Working directory: <?= $wd_link ?><?=$dir?><?= $wd_link2 ?> [<a href="<?=$_SERVER['SCRIPT_NAME']?>?action=changewdform&amp;dir=<?=$dir?>">change</a>]</td>
                                    ...
                                    if ($test_secured >= 1)
                                    {
                                        print "<td align=\"right\"><a href=\"{$_SERVER['SCRIPT_NAME']}?dir=$dir&amp;action=removehtaccessfileform\"><span class=\"ok\"><img src=\"{$_SERVER['SCRIPT_NAME']}?image=padlock\" alt=\"closed padlock icon\" border=0>Protected</span></a></td></tr>";
                                    } else
                                    {
                                        print "<td align=\"right\"><a href=\"{$_SERVER['SCRIPT_NAME']}?dir=$dir&amp;action=installhtaccessfile\"><span class=\"warn\"><img src=\"{$_SERVER['SCRIPT_NAME']}?image=padlock-open\" alt=\"open padlock icon\" border=0>Unprotected</span></a></td></tr>";
                                    }
                                    ?>
                        </table>
                    </td>
                ...
                function installhtaccessfile($dir)
                {
                    ...
                    $output = "# htpasstool v1.0 protected\nAuthType Basic\nAuthName \"Restricted Area\"\nAuthUserFile \"$dir/" . HTPASSWD . "\"\nrequire valid-user\n";
                    fwrite($fp,$output);
                    closefile($fp);
...
#    $output = preg_replace("/\n$/",'',$output);
                ...
                } elseif ($action == 'add')
                {
                    ...
                    $lines[$login]['password'] = (strtoupper (substr(PHP_OS, 0,3)) == 'WIN' )?$password: crypt($password,$pattern{rand(0,61)} . $pattern{rand(0,61)});
                    ...
                    {
                        $message = "User $login has been created successfully";
                    }
                ...
                } elseif ($action == 'assignpassword')
                {
                    ...
                    mail($sender,"(carbon copy): $subject",$body,$headers,$extra);
                ...
                } elseif ($action == 'delete')
                {
                    ...
                    unset($lines[$login]);
                    if (in_htpasstool_dir() && (count($lines) == 0))
                    {
                        closefile($fp);
                ...
                } elseif ($action == 'installhtaccessfile')
                {
                    ...
                    if (file_exists("$wdir/" . HTPASSWD)&&(!in_htpasstool_dir() || is_writable("$wdir/" . HTPASSWD)) && (filesize("$wdir/" . HTPASSWD) >= 15))
                    {
